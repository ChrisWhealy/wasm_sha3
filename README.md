...
What makes the SHA3 function fundamentally different from the earlier SHA functions, is that the "squeeze" operation can be performed an unlimited number of times; hence, when used as an extendable output function (XOF), the Keccak function can act as a psuedo-random number generator.

...
## SHA3 Internal State (AKA "Entropy Pool")
...
This internal state is treated as a 3-dimensional matrix having the dimensions `5 * 5 * w`, where `w = 2^l` and where `l` is an integer in the range `0..6`.

The size (`b`) of the internal state in bits may therefore be only one of:

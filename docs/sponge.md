...
What makes the SHA3 function fundamentally different from the earlier SHA1 or SHA2 functions, is that the "squeeze" operation can be performed an unlimited number of times; hence, when used in Extendible Output Function (XOF) mode, the SHA3 function can act as a pseudo-random number generator.
...
However, the squeeze phase can be run multiple times, thus allowing SHA3 to behave as a pseudo-random number generator.

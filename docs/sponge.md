...
If the squeeze phase runs just once, then the first `d` bits of the output value act as a drop-in replcement for a digest of `d` bits generated by the SHA2 function.

However, the squeeze phase can be run multiple times, thus behaving as a psuedo-random number generator.
